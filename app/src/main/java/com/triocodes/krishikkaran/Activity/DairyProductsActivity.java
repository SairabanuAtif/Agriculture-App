...
public class DairyProductsActivity extends ParentActivity implements VolleyCallback, Response.ErrorListener, Response.Listener {
    Toolbar mToolbar;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mListView = (ListView) this.findViewById(R.id.list_dairy);
        mLayoutNoDairy = (LinearLayout) this.findViewById(R.id.layout_no_dairy);
        mLayoutNoDairy.setVisibility(View.GONE);
    ...
    private void passData() {
        try {
            JSONObject mDairyDetails = new JSONObject();
            mDairyDetails.put("CategoryID", "7");
            mDairyDetails.put("CountFrom", x);
            mDairyDetails.put("CountTo", y);
            super.showProgress("Please Wait");
            establishConnection(Request.Method.POST, Constants.mDairyProductsUrl, mDairyDetails);
        } catch (Exception e) {
    ...
    private void handleDiaryList(String responseType) {
        try {
            ...
            for (int i = 0; i < mJsonArray.length(); i++) {
                ...
                VegetablesModel mDiaryModel = new VegetablesModel();
                mDiaryModel.setmProductName(mProductName);
                mDiaryModel.setmSellerAddress(mAddress);
                mDiaryModel.setmAmount(mAmount);
                mDiaryModel.setmQuantity(mQuantity);
                mDiaryModel.setmSellerName(mName);
                mDiaryModel.setmSellerMobile(mMobile);
                mDiaryModel.setmSellerEmail(mEmail);
                mDiaryModel.setmProductCode(mProductCode);
                mDiaryModel.setmQuantityUnit(mQuantityUnit);
                mDiaryModel.setmThumbnailUrl(mImageUrl);
                mDiaryModel.setTotalProducts(mTotalProducts);

                List.add(mDiaryModel);
            }
        } catch (Exception e) {
        ...
        try {
            if (List != null && List.size() > 0) {
                ...
                mLayoutNoDairy.setVisibility(View.VISIBLE);
            }
        } catch (Exception e) {
        }
    }
