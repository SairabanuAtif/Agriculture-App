...
public class SpicesActivity extends ParentActivity implements VolleyCallback,Response.Listener,Response.ErrorListener {
    Toolbar mToolbar;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mListView=(ListView)this.findViewById(R.id.list_spices);
        mLayoutNoSpices=(LinearLayout)this.findViewById(R.id.layout_no_spices);
        mLayoutNoSpices.setVisibility(View.GONE);
    ...
    private void passData() {
        try {
            ...
            mVegetableDetails.put("CategoryID", "3");
            mVegetableDetails.put("CountFrom",x);
            mVegetableDetails.put("CountTo",y);
            super.showProgress("Please Wait");
            establishConnection(Request.Method.POST, Constants.mSpicesUrl, mVegetableDetails);
        } catch (Exception e) {
    ...
    private void handleSpiceList(String responseType) {
        try{
            ...
            for(int i=0;i<mJsonArray.length();i++){
                ...
                VegetablesModel mSpicesModel=new VegetablesModel();
                mSpicesModel.setmProductCode(mProductCode);
                mSpicesModel.setmProductName(mProductName);
                mSpicesModel.setmSellerAddress(mAddress);
                mSpicesModel.setmSellerMobile(mMobile);
                mSpicesModel.setmSellerEmail(mEmail);
                mSpicesModel.setmQuantity(mQuantity);
                mSpicesModel.setmAmount(mAmount);
                mSpicesModel.setmSellerName(mName);
                mSpicesModel.setmQuantityUnit(mQuantityUnit);
                mSpicesModel.setmThumbnailUrl(mImageUrl);
                mSpicesModel.setTotalProducts(mTotalProducts);
                List.add(mSpicesModel);
            }
        }catch (Exception e){
        ...
        try{
            if(List!=null&&List.size()>0){
                ...
                mLayoutNoSpices.setVisibility(View.VISIBLE);
            }
        }catch (Exception e){
