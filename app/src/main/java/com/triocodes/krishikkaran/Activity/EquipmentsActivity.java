...
public class EquipmentsActivity extends ParentActivity implements VolleyCallback,Response.ErrorListener,Response.Listener {
    Toolbar mToolbar;
    ...
    ListView mListView;
    LinearLayout mLayoutNoEquipments;
    View footer, header;
    int x = 1;
    int y = 12;
    int totalItem=0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mListView = (ListView) this.findViewById(R.id.list_equipment);
        mLayoutNoEquipments=(LinearLayout)this.findViewById(R.id.layout_no_equipment);
        mLayoutNoEquipments.setVisibility(View.GONE);
    ...
    private void passData() {
        try {
            ...
            mVegetableDetails.put("CategoryID", "9");
            mVegetableDetails.put("CountFrom",x);
            mVegetableDetails.put("CountTo",y);
            super.showProgress("Please Wait");
            establishConnection(Request.Method.POST, Constants.mEquipmentsUrl, mVegetableDetails);
        } catch (Exception e) {
            ...
            public void run() {
                if (!extras.isEmpty()) {
                    try {
                        ...
                        handleEquipmentsList(responseType);
                    } catch (Exception e) {
    ...
    private void handleEquipmentsList(String responseType) {
        try {
            ...
            for (int i = 0; i < mJsonArray.length(); i++) {
                ...
                VegetablesModel mEquipmentsModel = new VegetablesModel();
                mEquipmentsModel.setmProductName(mProductName);
                mEquipmentsModel.setmSellerAddress(mAddress);
                mEquipmentsModel.setmAmount(mAmount);
                mEquipmentsModel.setmQuantity(mQuantity);
                mEquipmentsModel.setmSellerName(mName);
                mEquipmentsModel.setmSellerMobile(mMobile);
                mEquipmentsModel.setmSellerEmail(mEmail);
                mEquipmentsModel.setmProductCode(mProductCode);
                mEquipmentsModel.setmQuantityUnit(mQuantityUnit);
                mEquipmentsModel.setmThumbnailUrl(mImageUrl);
                mEquipmentsModel.setTotalProducts(mTotalProducts);

                List.add(mEquipmentsModel);
            }
        } catch (Exception e) {
        ...
        try {
            if (List != null && List.size() > 0) {
                ...
                mLayoutNoEquipments.setVisibility(View.VISIBLE);
            }
        } catch (Exception e) {
        }
