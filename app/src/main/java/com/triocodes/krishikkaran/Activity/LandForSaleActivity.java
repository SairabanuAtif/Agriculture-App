...
public class LandForSaleActivity extends ParentActivity implements VolleyCallback,Response.ErrorListener,Response.Listener {
    Toolbar mToolbar;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mListView = (ListView) this.findViewById(R.id.list_land_for_rent);
        mLayoutNoLand=(LinearLayout)this.findViewById(R.id.layout_no_land);
        mLayoutNoLand.setVisibility(View.GONE);
    ...
    private void passData() {
        try {
            ...
            mVegetableDetails.put("CategoryID", "11");
            mVegetableDetails.put("CountFrom",x);
            mVegetableDetails.put("CountTo",y);
            super.showProgress("Please Wait");
            establishConnection(Request.Method.POST, Constants.mLandForRentUrl, mVegetableDetails);
        } catch (Exception e) {
            ...
            public void run() {
                if (!extras.isEmpty()) {
                    try {
                        ...
                        handleLandForRentList(responseType);
                    } catch (Exception e) {
    ...
    private void handleLandForRentList(String responseType) {
        try {
        ...
        try {
            if (List != null && List.size() > 0) {
                ...
                mLayoutNoLand.setVisibility(View.VISIBLE);
            }
        } catch (Exception e) {
