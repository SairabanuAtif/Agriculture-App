...
public class AnimalsActivity extends ParentActivity implements VolleyCallback,Response.ErrorListener,Response.Listener {
    Toolbar mToolbar;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mListView = (ListView) this.findViewById(R.id.list_animal);
        mLayoutNoAnimals=(LinearLayout)this.findViewById(R.id.layout_no_animal);
        mLayoutNoAnimals.setVisibility(View.GONE);
    ...
    private void passData() {
        try {
            ...
            mVegetableDetails.put("CategoryID", "8");
            mVegetableDetails.put("CountFrom",x);
            mVegetableDetails.put("CountTo",y);
            super.showProgress("Please Wait");
            establishConnection(Request.Method.POST, Constants.mAnimalsUrl, mVegetableDetails);
        } catch (Exception e) {
            ...
            public void run() {
                if (!extras.isEmpty()) {
                    try {
                        ...
                        handleAnimalsList(responseType);
                    } catch (Exception e) {
    ...
    private void handleAnimalsList(String responseType) {
        try {
            ...
            for (int i = 0; i < mJsonArray.length(); i++) {
                ...
                VegetablesModel mAnimalsModel = new VegetablesModel();
                mAnimalsModel.setmProductName(mProductName);
                mAnimalsModel.setmSellerAddress(mAddress);
                mAnimalsModel.setmAmount(mAmount);
                mAnimalsModel.setmQuantity(mQuantity);
                mAnimalsModel.setmSellerName(mName);
                mAnimalsModel.setmSellerMobile(mMobile);
                mAnimalsModel.setmSellerEmail(mEmail);
                mAnimalsModel.setmProductCode(mProductCode);
                mAnimalsModel.setmQuantityUnit(mQuantityUnit);
                mAnimalsModel.setmThumbnailUrl(mImageUrl);
                mAnimalsModel.setTotalProducts(mTotalProducts);

                List.add(mAnimalsModel);
            }
        } catch (Exception e) {
        ...
        try {
            if (List != null && List.size() > 0) {
                ...
                mLayoutNoAnimals.setVisibility(View.VISIBLE);
            }
        } catch (Exception e) {
        }
    }
