...
public class FruitsActivity extends ParentActivity implements VolleyCallback, Response.ErrorListener, Response.Listener {
    Toolbar mToolbar;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mListView = (ListView) this.findViewById(R.id.list_fruit);
        mLayoutNoFruits = (LinearLayout) this.findViewById(R.id.layout_no_fruits);
        mLayoutNoFruits.setVisibility(View.GONE);
    ...
    private void passData() {
        try {
            ...
            mVegetableDetails.put("CategoryID", "2");
            mVegetableDetails.put("CountFrom", x);
            mVegetableDetails.put("CountTo", y);
            super.showProgress("Please Wait");
            establishConnection(Request.Method.POST, Constants.mFruitsUrl, mVegetableDetails);
        } catch (Exception e) {
    ...
    private void handleFruitsList(String responseType) {
        try {
            ...
            for (int i = 0; i < mJsonArray.length(); i++) {
                ...
                VegetablesModel mFruitsModel = new VegetablesModel();
                mFruitsModel.setmProductName(mProductName);
                mFruitsModel.setmSellerAddress(mAddress);
                mFruitsModel.setmAmount(mAmount);
                mFruitsModel.setmQuantity(mQuantity);
                mFruitsModel.setmSellerName(mName);
                mFruitsModel.setmSellerMobile(mMobile);
                mFruitsModel.setmSellerEmail(mEmail);
                mFruitsModel.setmProductCode(mProductCode);
                mFruitsModel.setmQuantityUnit(mQuantityUnit);
                mFruitsModel.setmThumbnailUrl(mImageUrl);
                mFruitsModel.setTotalProducts(mTotalProducts);

                List.add(mFruitsModel);
            }
        } catch (Exception e) {
        ...
        try {
            if (List != null && List.size() > 0) {
                ...
                mLayoutNoFruits.setVisibility(View.VISIBLE);
            }
        } catch (Exception e) {
