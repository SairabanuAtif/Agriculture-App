...
public class FlowersActivity extends ParentActivity implements VolleyCallback, Response.ErrorListener, Response.Listener {
    Toolbar mToolbar;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mListView = (ListView) this.findViewById(R.id.list_flowers);
        mLayoutNoFlowers = (LinearLayout) this.findViewById(R.id.layout_no_flowers);
        mLayoutNoFlowers.setVisibility(View.GONE);
    ...
    private void passData() {
        try {
            ...
            mVegetableDetails.put("CategoryID", "5");
            mVegetableDetails.put("CountFrom", x);
            mVegetableDetails.put("CountTo", y);
            super.showProgress("Please Wait");
            establishConnection(Request.Method.POST, Constants.mFlowersUrl, mVegetableDetails);
        } catch (Exception e) {
            ...
            public void run() {
                if (!extras.isEmpty()) {
                    try {
                        ...
                        handleFlowersList(responseType);
                    } catch (Exception e) {
    ...
    private void handleFlowersList(String responseType) {
        try {
            ...
            for (int i = 0; i < mJsonArray.length(); i++) {
                ...
                VegetablesModel mFlowersModel = new VegetablesModel();
                mFlowersModel.setmProductName(mProductName);
                mFlowersModel.setmSellerAddress(mAddress);
                mFlowersModel.setmAmount(mAmount);
                mFlowersModel.setmQuantity(mQuantity);
                mFlowersModel.setmSellerName(mName);
                mFlowersModel.setmSellerMobile(mMobile);
                mFlowersModel.setmSellerEmail(mEmail);
                mFlowersModel.setmProductCode(mProductCode);
                mFlowersModel.setmQuantityUnit(mQuantityUnit);
                mFlowersModel.setmThumbnailUrl(mImageUrl);
                mFlowersModel.setTotalProducts(mTotalProducts);

                List.add(mFlowersModel);
            }
        } catch (Exception e) {
        ...
        try {
            if (List != null && List.size() > 0) {
                ...
                mLayoutNoFlowers.setVisibility(View.VISIBLE);
            }
        } catch (Exception e) {
        }
    }
