...
public class HandloomActivity extends ParentActivity implements VolleyCallback,Response.ErrorListener,Response.Listener {
    Toolbar mToolbar;
    ...
    ListView mListView;
    LinearLayout mLayoutNoHandloom;
    View footer, header;
    int x = 1;
    int y = 12;
    int totalItem=0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mListView = (ListView) this.findViewById(R.id.list_handloom);
        mLayoutNoHandloom=(LinearLayout)this.findViewById(R.id.layout_no_handloom);
        mLayoutNoHandloom.setVisibility(View.GONE);
    ...
    private void passData() {
        try {
            ...
            mVegetableDetails.put("CategoryID", "10");
            mVegetableDetails.put("CountFrom",x);
            mVegetableDetails.put("CountTo",y);
            super.showProgress("Please Wait");
            establishConnection(Request.Method.POST, Constants.mHandloomUrl, mVegetableDetails);
        } catch (Exception e) {
            ...
            public void run() {
                if (!extras.isEmpty()) {
                    try {
                        ...
                        handleHandloomList(responseType);
                    } catch (Exception e) {
    ...
    private void handleHandloomList(String responseType) {
        try {
            ...
            for (int i = 0; i < mJsonArray.length(); i++) {
                ...
                VegetablesModel mHandloomModel = new VegetablesModel();
                mHandloomModel.setmProductName(mProductName);
                mHandloomModel.setmSellerAddress(mAddress);
                mHandloomModel.setmAmount(mAmount);
                mHandloomModel.setmQuantity(mQuantity);
                mHandloomModel.setmSellerName(mName);
                mHandloomModel.setmSellerMobile(mMobile);
                mHandloomModel.setmSellerEmail(mEmail);
                mHandloomModel.setmProductCode(mProductCode);
                mHandloomModel.setmQuantityUnit(mQuantityUnit);
                mHandloomModel.setmThumbnailUrl(mImageUrl);
                mHandloomModel.setTotalProducts(mTotalProducts);

                List.add(mHandloomModel);
            }
        } catch (Exception e) {
        ...
        try {
            if (List != null && List.size() > 0) {
                ...
                mLayoutNoHandloom.setVisibility(View.VISIBLE);
            }
        } catch (Exception e) {
        }
    }
